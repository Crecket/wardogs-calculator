...
```
   ...
   deployed site means uploading the tile pyramid to R2 under a versioned
   release prefix and setting `tiles.path` to its full public HTTPS URL; local
   tiles are excluded from the production build: see
   [Uploading tiles](deployment.md#uploading-tiles).

4. If the map needs an indexable guide, add a unique, fact-checked definition to `scripts/map-landing-pages.mjs`, link it from the relevant English homepage SEO section, then run the build and SEO smoke test. Do not publish mechanical translations or copy another map's text with only the name changed.
