...
8. [x] ~~**Plan export as an image.** Canvas to PNG with a legend of guns, targets and solutions, sized for a Discord post. People already do this badly with the print-screen key; the distribution channel for this whole genre is the screenshot.~~
9. [x] **Tile decode via `createImageBitmap`.** `js/map/tiles.js` uses `new Image()`, so decode lands on the main thread. Pair it with an explicit-`close()` LRU to bound bitmap retention.
10. [x] **Cap `devicePixelRatio` at 2.** `resize()` multiplies by the full ratio; on a DPR-3 phone that is 2.25× the pixels for no visible gain, on the device with the least headroom.
