...
28. [ ] **Node reuse on the pointer-move path.** The flight-time badges rebuild DOM on every `result()`, already flagged in [extraction-plan.md](../../extraction-plan.md). Worth generalising into a rule rather than fixing the one site.
