...
`canopy.json` + `canopy.png` mark which 8 m cells of the playable bounds are under trees, generated by `npm run build-canopy` and committed. The terrain chunks are landscape collision and carry nothing that stands on the ground, so without this a flat forest floor is indistinguishable from a field.

The source is the finest zoom of the map's tile pyramid, read from `maps/tiles/<map-id>/zoom_<maxZoom>/`, which is git-ignored and normally served from the bucket. Fetch the tiles over the playable bounds before running the bake; a map without a local copy is skipped. The tiles are decoded through the same headless Chromium `scripts/bench-render.mjs` uses, so `playwright-core` has to be installed.

The tiles are monochrome, so a cell is canopy when its patch of pixels is both dark and rough. Dark and smooth is a road or water, bright is a field or the ground above the treeline. The thresholds live in `scripts/lib/canopy.mjs` and were set by eye on Bakurani; the raster is a reading of the picture, not of the game.

`npm run build-firing-positions` refuses cells inside canopy and lifts the 32 m heightfield by 30 m wherever a heightfield cell is mostly wood, so the clearance march sees a thick wood as an obstacle in the shell's lane. The app's own shot assessment does not see canopy; the layer is stricter than the click verdict by design.
...
except `zh-cn`, whose entries live directly in that same `UI_TEXT` object.
