...
2. Dead ground asynchronous, worker-backed. `js/map/dead-ground.js` `terrainDeadGround`, `js/features/reach-badges.js` `reachSolveGun`. 110 ms at 1x to 672 ms at 6x per 8 m of gun drag with the layer on. Design in the section above. Medium size, medium risk because the reach badges read the cache synchronously.
