...
- **D** — closed. The flat range gate is gone: `rangeStatus`, the terrain note and the rings all read the same anchored per-arc gates, `declared + (model(ΔZ) − model(0))`, so the panel and the ring answer the same question with the same arithmetic. The ring aggregates those gates across arcs (max of `arcMaxRangeModel`, min of `arcMinRangeModel`) exactly as `assessShot`'s verdict takes the best arc, which is what makes the two agree rather than merely resemble each other. Where the model term is undefined at a given ΔZ — the SPG low arc's shallow stop cannot reach a target more than about 75 m above the gun, so `arcMinRangeModel` returns null there — the gate falls back to the declared bound rather than disappearing, so the declared `minRange`/`maxRange` remain the floor at every ΔZ. What the ring still does not draw is masking: a target that every arc reaches but terrain hides is inside the drawn ring and reported `masked` by the panel, which is a division of labour with the dead-ground wedges, not a disagreement.
