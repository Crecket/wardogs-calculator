...
**The vertical scale is piecewise, and says so.** A high arc's apex is an order of magnitude above the terrain relief — +1300 m of sky over 200 m of ground — so a linear axis squashes the profile into a sliver. `crossSectionScale()` guarantees the terrain band a third of the plot height and compresses everything above the flat arc's apex into the remainder, breaking the axis at that level. When it breaks it draws the break line, labels its height above the gun, and prints "scale compressed above" in the corner, so nobody reads the high arc's shape as literal. When the natural proportions already give the terrain its third, the axis stays linear and unmarked. The profile is also sampled 5% beyond each end, so the ground around the gun and around the target is drawn rather than clipped against the frame.
