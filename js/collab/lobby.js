...
async function initLobby() {
    ...
    const words = {
        en: {
            ...
            include: 'Share my saved targets too', privacy: 'Points, weapon and drawings are shared. Anyone with the link can edit. Camera and locks stay local.',
            copy: 'Copy invite', leave: 'Leave', close: 'Close for everyone', reconnect: 'Reconnect', export: 'Export recovery copy',
            idle: 'No connection until you create or join a lobby.', connecting: 'Connecting…', ready: 'Connected · all changes saved',
        ...
        ru: {
            ...
            include: 'Поделиться и моими сохранёнными целями', privacy: 'Точки, оружие и рисунки общие. Любой со ссылкой может редактировать. Камера и блокировки остаются личными.',
            ...
            idle: 'Подключение начнётся только после создания лобби или входа.', connecting: 'Подключение…', ready: 'Подключено · изменения сохранены',
    ...
    let batchTimer, connectTimer, ackTimer, heartbeat;
    ...
    const rawDocument = (includeSaved = true) => ({
        mapId: S.map, w: S.w, h: S.h, origin: S.origin, target: S.target, weapon: S.weapon,
    ...
    function validDocument(raw) {
        ...
        if ((doc.mapId !== 'custom' && !Object.hasOwn(MAPS, doc.mapId)) || (doc.weapon && !Object.hasOwn(WEAPONS, doc.weapon))) throw new Error('unsupported-room');
    ...
    function render() {
        ...
        try {
            ...
            Object.assign(S, { map: doc.mapId, w: doc.w, h: doc.h, origin: structuredClone(doc.origin), target: structuredClone(doc.target), weapon: doc.weapon });
            if (different) Object.assign(S, { zoom: 1, panX: 0, panY: 0 });
    ...
    function clearTimers() {
        clearTimeout(batchTimer); clearTimeout(connectTimer); clearTimeout(ackTimer); clearInterval(heartbeat);
        batchTimer = null;
    }
    function preserve() {
    ...
    function leave(force = false) {
        ...
        replica = null; observed = null; code = ''; ownerKey = ''; roster = []; readOnly = false; needsRender = false;
        updateUI();
    }
            ...
            try {
                ...
                if (msg.type === 'snapshot') {
                    ...
                    if (joining) {
                        ...
                        current.send(JSON.stringify({ type: 'name', name: q('.lobby-name').value }));
                    ...
                    you = msg.you; roster = msg.roster; maximum = msg.maxParticipants; expiresAt = msg.expiresAt; remaining = msg.remainingUpdates;
                    ...
                    if (!replica.flight) clearTimeout(ackTimer);
                    render(); schedule();
                } else if (msg.type === 'peers') { roster = msg.roster; updateUI(); }
    ...
    function exportCopy() {
        ...
        if (!doc) return;
        // Existing map/target importers understand data/targets. Extra fields preserve gun/map settings for reference.
    ...
    lobby = { active: false, capture, updateUI, updateHistoryUI, undo: () => history('undo'), redo: () => history('redo') };
